...
export class Cache implements CacheInterface {
  ...
  constructor(options: CacheOptionsInterface) {
    ...
    this._asyncIncr = promisify(this.client.incr).bind(this.client);
  ...
  async keys(glob="*"): Promise<string[]> {
  ...
  async increment(key:string): Promise<string> {
    enforceArgumentType('key', key, 'string');
    return await this._asyncIncr(key);
  }
